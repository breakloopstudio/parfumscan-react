...
import { OBSTACLE_DEFS, OBSTACLE_POOL_SIZE } from './runner-types';
...
function Slot({ slot, groundY }: { slot: ObsSlot; groundY: number }) {
  ...
  const pos = useAnimatedStyle(() => {
    ...
    return { transform: [{ translateX: x.value }, { translateY: groundY - d.height }], opacity: a.value ? 1 : 0 };
  });
...
export default function RunnerObstacles({ obs, groundY }: Props) {
