    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.bg}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        <View style={s.form}>
          ...
          <View style={s.inputGroup}>
            <TextInput style={s.input} placeholder="6 caractères minimum" placeholderTextColor={theme.colors.medium}
          ...
          <Pressable style={[s.submitBtn, (!canSubmit || isLoading) && s.submitBtnDisabled]}
            onPress={handleEmailRegister} disabled={!canSubmit || isLoading}>
            {loading === 'email' ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={s.submitText}>Créer mon compte</Text>}
          </Pressable>
